...
//    [self.webView setCustomUserAgent:@"Mozilla/5.0 (iPod; U; CPU like Mac OS X; en) AppleWebKit/420.1 (KHTML, like Gecko) Version/3.0 Mobile/3A100a Safari/419.3"];
...
- (void)webView:(WebView *)webView decidePolicyForNavigationAction:(NSDictionary *)actionInformation request:(NSURLRequest *)request frame:(WebFrame *)frame decisionListener:(id<WebPolicyDecisionListener>)listener
{
    ...
    if (!hasDoneFinalRedirect_) {
        hasDoneFinalRedirect_ = [self.signIn requestRedirectedToRequest:request];
        if (hasDoneFinalRedirect_) {
//            if ([[request.URL absoluteString] isEqualToString:self.authentication.redirectURI]) {
//                [self destroyWindow];
//                [self handlePrematureWindowClose];
//            }
